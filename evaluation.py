...
from helper import catch_streaming_response, exact_match_retrieval, get_retriever
...
async def evaluate_retrieval(test_data,k):
    ...
    for i in range(len(test_data)):
        ...
        retrieved_context = await retrieve_docs(user_input,retriever, k)
...
async def evaluate_generation(test_data,k, temperature=0.1, model_name="gpt-4o-mini"):
    ...
    for i in range(len(test_data)):
        user_input = test_data["user_input"].loc[i]
        docs = await retrieve_docs(user_input, retriever, k)
        docs_tuple= [(doc.id, doc.page_content) for doc in docs]
        docs_text = [doc.page_content for doc in docs]
        response, t1, t2, t3 = await catch_streaming_response(generate_answer, user_input, docs_tuple, temperature, model_name)
...
async def full_evaluation(test_data, k=3, temperature=0.1, model_name="gpt-4o-mini"):
    ...
    for i in range(len(test_data)):
        ...
        #Generation
        docs_text = [doc.page_content for doc in docs]
        response,t_startg,t_first_g,t_last_g = await catch_streaming_response(generate_answer,user_input, docs_text, temperature, model_name)
